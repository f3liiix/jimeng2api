...
import { resolveVideoModel } from "@/api/models/registry.ts";
import { getOmniVideoBenefitType, getVideoBenefitType } from "@/api/models/video-benefits.ts";
import { hasOmniReferenceMaterials } from "@/api/models/video-request-validation.ts";

export const DEFAULT_MODEL = DEFAULT_VIDEO_MODEL;
const MAX_REMOTE_IMAGE_BYTES = 100 * 1024 * 1024;
const REMOTE_DOWNLOAD_TIMEOUT_MS = 60000;

export function getModel(model: string, regionInfo: RegionInfo) {
  return resolveVideoModel(model, regionInfo);
}
...
export async function generateVideo(
  ...
  const useOmniPayload = isOmniMode && hasOmniReferenceMaterials({
  ...
  if (useOmniPayload) {
    ...
    // 根据模型选择 benefit_type
    const omniBenefitType = getOmniVideoBenefitType(model);
