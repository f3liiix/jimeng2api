...
class Server {
    ...
    constructor() {
        ...
        this.app.use(async (ctx: any, next: Function) => {
            if (ctx.is('application/json') && ['POST', 'PUT', 'PATCH'].includes(ctx.method)) {
                ...
                const limitBytes = parseByteSize(config.system.requestBody?.jsonLimit, 100 * 1024 * 1024);
                const { body: parsedBody, rawBody } = await parseJsonBody(ctx.req, limitBytes);
    ...
    attachRoutes(routes: any[]) {
        ...
        this.app.use((ctx: any) => {
            ...
            logger.warn(message);
            const failureBody = new FailureBody(new Exception(EX.SYSTEM_NOT_ROUTE_MATCHING, message).setHTTPStatusCode(404));
            const response = new Response(failureBody);
